...
#define SUPERBLOCK_SECTOR	(SUFS_SUPERBLOCK_OFFSET / disk->sector_size)
...
void sufs_mount(disk_t* _disk)
{
	...
	block_buf = Aligned_alloc(sizeof(struct sufs_dinode), sb.sb_block_size);
	map_block_buf = Malloc(sb.sb_block_size);
	indirect_block_buf = Malloc(sb.sb_block_size);
...
ssize_t sufs_write(struct sufs_dinode* inode, void* data, uint64_t offset, size_t nbytes)
{
	...
	if (offset % sb.sb_block_size > 0) {
		...
		disk_read_block(disk, block_buf, block_idx);
		memcpy(block_buf + block_offset, data, to_write);
		disk_write_block(disk, block_buf, block_idx);
	...
	if (nbytes > 0) {
		...
		disk_read_block(disk, block_buf, end_block_idx);
...
void sufs_dump_dir_tree(void)
{
	...
	struct dir* curr_dir = Malloc(sizeof(struct dir));
	...
	void* my_block_buf = Aligned_alloc(sizeof(struct sufs_dentry), sb.sb_block_size);
	...
	while (dir_stack.size > 0) {
		...
		for (uint32_t i = 0; i < curr_dir->inode.di_nblocks; i++) {
			disk_read_block(disk, my_block_buf, get_data_block(inode, i, false););
...
static void write_superblock()
{
	...
	disk_write_sector(disk, &sb, SUPERBLOCK_SECTOR);
}
...
static void write_inode(const struct sufs_dinode* inode)
{
	...
	disk_read_block(disk, block_buf, inode_block);
	memcpy(block_buf + block_offset, inode, sizeof(struct sufs_dinode));
	disk_write_block(disk, block_buf, inode_block);
}
...
static uint32_t search_dir(const struct sufs_dinode* dir_inode, const char* name)
{
	for (uint32_t i = 0; i < dir_inode->di_nblocks; i++) {
		disk_read_block(disk, block_buf, get_data_block(dir_inode, i));
	...
	for (i = 0; i < dir_inode->di_nblocks; i++) {
		...
		for (uint32_t j = 0; j < sb.sb_dentpb; j++) {
			if (dentries[j].de_inum == inum) {
				...
				if (i > 0) {	// Free block if it becomes empty
...
static void remove_dir_dblock(struct sufs_dinode* dir_inode, uint32_t idx)
{
	...
	if (idx < dir_inode->di_nblocks - 1) {
		disk_read_block(disk, block_buf, last_block_idx);
		disk_write_block(disk, block_buf, get_data_block(dir_inode, idx));
	}
...
static int create_file(char* path, int mode)
{
	...
	if (mode & IFDIR) {
		...
		disk_write_block(disk, block_buf, inode.di_db[0]);
	}
...
static int delete_file(char* path, bool is_dir)
{
	...
	if (is_dir) {
		...
		disk_read_block(disk, block_buf, inode.di_db[0]);
		struct sufs_dentry* dentries = block_buf;
...
static struct sufs_dinode* iget(uint32_t inum, struct sufs_dinode* iout)
{
	...
	if (iout == NULL)
		iout = Malloc(sizeof(struct sufs_dinode));
...
static struct sufs_dinode* ialloc(struct sufs_dinode* iout)
{
	...
	for (i = 0; i < sb.sb_inode_map_bsize; i++) {
		disk_read_block(disk, map_block_buf, sb.sb_inode_map_boff + i);
	...
	disk_write_block(disk, map_block_buf, sb.sb_inode_map_boff + i);
...
static void ifree(uint32_t inum)
{
	...
	disk_read_block(disk, map_block_buf, block_index);
	bitmap_free(map_block_buf, bit_offset, 1);
	disk_write_block(disk, map_block_buf, block_index);
...
static uint32_t dballoc(void)
{
	...
	for (i = 0; i < sb.sb_dblock_map_bsize; i++) {
		disk_read_block(disk, map_block_buf, sb.sb_dblock_map_boff + i);
	...
	disk_write_block(disk, map_block_buf, sb.sb_dblock_map_boff + i);
...
static void dbfree(uint32_t block_idx)
{
	...
	disk_read_block(disk, map_block_buf, map_block);
	bitmap_free(map_block_buf, bit_offset, 1);
	disk_write_block(disk, map_block_buf, map_block);
