...
static HMNode* create_node(char* key, value_type value) {
...
static HMNode* get_bucket(HashMap* hash_map, char* key) {
    return hash_map->buckets + (hash(key) % hash_map->capacity);
}

static HMNode* get_node(HashMap* hash_map, char* key) {
    HMNode* bucket = get_bucket(hash_map, key);
    while (strcmp(bucket->key, key))
        bucket = bucket->next;
    return bucket;
}
...
value_type hm_replace(HashMap* hash_map, char* key, value_type value) {
    ...
    value_type old_value = node->value;
    node->value = value;
    return old_value;
